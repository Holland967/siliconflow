...
def text_chatting(api_key: str, base_url: str) -> None:
    from model_config import text_chat_models
    from chat_completion import ChatCompletion
    from template import text_chat_default_prompt

    mode: str = "text_chat"
    model_list: list = text_chat_models(api_key=api_key)
    
    chat = ChatCompletion(api_key=api_key, base_url=base_url)

    set_session_state(
        mode=mode,
        prompt=text_chat_default_prompt,
        tokens=4096,
        temp=0.70)
    ...
    if query := st.chat_input("Say something...", key=f"{mode}_query", disabled=not model):
    ...
    if clear_btn:
        st.session_state[f"{mode}_sys"] = text_chat_default_prompt
