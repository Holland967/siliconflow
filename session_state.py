...
def set_session_state(mode: str, prompt: str, tokens: int, temp: float):
    if f"{mode}_sys" not in st.session_state:
        st.session_state[f"{mode}_sys"] = prompt
    ...
    if f"{mode}_retry" not in st.session_state:
        st.session_state[f"{mode}_retry"] = []
    ...
    if f"{mode}_topp" not in st.session_state:
        st.session_state[f"{mode}_topp"] = 0.9
